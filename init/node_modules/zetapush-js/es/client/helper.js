...
class ClientHelper {
  ...
  constructor({
    ...
    authentication,
    resource = null,
    transports = _zetapushCometd.Transports
  }) {
    /**
     * @access private
     * @type {string}
     ...
     * @type {Promise}
     */
    this.servers = (0, _index.getServers)({
      apiUrl,
      sandboxId,
      forceHttps,
      transports
  ...
  connect() {
    this.servers.then(servers => {
      if (servers.length > 0) {
        ...
        this.serverUrl = (0, _index.shuffle)(servers);
        // Configure CometD
  ...
  createService({ listener, Type, deploymentId = Type.DEFAULT_DEPLOYMENT_ID }) {
    const isMacroType = (0, _index.isDerivedOf)(Type, _services.Macro);
    const prefix = `/service/${this.getSandboxId()}/${deploymentId}`;
